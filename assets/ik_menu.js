;(function ( $, window, document, undefined ) {
	...
	var pluginName = 'ik_menu',
		defaults = {
            'instructions': 'Use arrow keys to navigate between  items, spacebar to expand submenus, escape  to close submenus, and enter to activate menu items.'
        };
	...
	Plugin.prototype.init = function () {
		...
		$elem.find('li>ul').attr({
            'role': 'menu',
            'aria-hidden': true // hide submenus
        });
	...
	Plugin.prototype.showSubmenu = function(event) {
		...
		if ($submenu.length) {
			$elem.addClass('expanded')
                .attr({'aria-expanded': true, tabindex: -1});
